<script lang="ts">
	...
	const darkThemeSource: Theme = JSON5.parse(
		`// ダークテーマのベーステーマ
  // このテーマが直接使われることは無い
  {
    id: 'dark',
  ...
  `
	...
	const darkThemeCompiled = compile(darkThemeSource);

	const lightThemeSource: Theme = JSON5.parse(
		`// ライトテーマのベーステーマ
  // このテーマが直接使われることは無い
  {
    id: 'light',
  ...
  `
	...
	const lightThemeCompiled = compile(lightThemeSource);
	...
	function applyTheme(theme: Theme) {
		...
		if (_theme.base) {
			const base = [lightThemeCompiled, darkThemeCompiled].find((x) => x.id === _theme.base);
